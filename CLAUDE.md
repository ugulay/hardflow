...
through the new pure `core/preview_cache`. Code is syntax-verified with pure +
headless tests (76 pure + 106 headless, run live against a standalone `bpy`
...
| `ui/draw.py` | GPU + blf helpers: shapes/points/fills/grid, the **framed HUD** (`draw_hud` with a bordered panel + optional accent `title`/header — every modal tool's premium overlay), and the viewport-guide primitives (`draw_rect_outline`, `draw_guide_line`, `draw_dashed_line`, `draw_snap_ring`, `draw_mirror_plane`, `fade_color` for translucent / fade-in overlays) |
