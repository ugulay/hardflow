...
| `operators/draw_cut.py` | Main modal drawing operator (`HARDFLOW_OT_draw`): cut/slice/make/**intersect**/face/**knife** (mode via `Tab`/`Shift+Tab`), **numeric exact-size entry** (type a dimension -> `_apply_numeric`/`grid.lock_distance`), plane cycling VIEW/SURFACE/**EDGES**(grid on selected edges, longest-edge main via `best_edge_pair`)/X/Y/Z (edge- and face-aligned tangents), `Shift+←/→` in-plane grid rotation (`_apply_spin`), `Z` quick-close, measurement HUD, live 3D cutter cage, Edit-Mode path (v1.3), and **in-draw ops** (v1.4/v1.6: inset `-/=`, rotate `,/.`, array `A`/axis `D`, mirror `M`, bevel-on-cut `B`, **bevelled cutter `C`**, stamp `G`, live grid Ctrl+Wheel, live depth PgUp/Dn) via `_processed_corner_sets` |
