...
| `operators/edge_tool.py` | Object-Mode **Edge Bevel** (on `face_tool._FaceDragModal`): raycast → pick nearest edge (`nearest_edge_on_face`) → drag width / `[ ]` segments → `bevel_object_edges` w/ live snapshot/restore; `R` **repeat** last width. Bevel an edge without Edit Mode (`HARDFLOW_OT_edge_bevel`) |
