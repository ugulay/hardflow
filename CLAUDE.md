...
| `operators/offset.py` | SketchUp Offset (on `face_tool._FaceDragModal`): drag to inset a face's border, bmesh inset w/ live snapshot/restore; `E` **chains into extruding** the inner face (recess / raised panel, two-phase `inset_extrude_faces`, depth has vertex/edge inference); `R` **repeat** last thickness (`HARDFLOW_OT_offset`) |
| `operators/edge_tool.py` | Object-Mode **Edge Bevel** (on `face_tool._FaceDragModal`): raycast → pick nearest edge (`nearest_edge_on_face`) → drag width / `[ ]` segments / `L` whole-loop (`edge_loop`) → `bevel_object_edges` w/ live snapshot/restore; `R` **repeat** last width. Bevel an edge (or loop) without Edit Mode (`HARDFLOW_OT_edge_bevel`) |
