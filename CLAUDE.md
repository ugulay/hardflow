...
| `core/path.py` | Pure poly-line / freehand-stroke math, stdlib only — the Curves-upgrade anchor (v1.21) (`path_length`, `dedup_points`, `rdp_simplify` (Ramer–Douglas–Peucker stroke → clean anchors; segment-distance so doubled-back strokes survive), `chaikin_smooth` (corner cutting, open endpoints kept / closed wrap), `catmull_rom` (centripetal Catmull-Rom THROUGH the anchors, open ghost-endpoint / closed; the Smooth Path spline), `resample_path` (even arc-length re-sampling, open keeps endpoints / closed drops the seam)). Consumed by `operators/pipe.py` |
...
| `operators/pipe.py` | Surface-snapping curve draw on the shared `_CurveDraw` modal (profile cycle via `_PROFILE_CYCLE`, P): pipe (drapes, F toggles; round/square/rect) + free-hanging sagging cable/rope + **Sweep / Follow-Me** (sweeps an L/U/T/I/box structural section along the path); live preview (curve or swept mesh). **v1.21 input upgrade:** implicit **click-or-stroke** (LMB drag past a px gate = freehand stroke sampled through the snap chain, reduced to anchors on release via `path.rdp_simplify`; Backspace undoes a whole stroke via `_groups`) + **`C` Smooth Path** (`path.catmull_rom` through the anchors; ROUND + Follow-off commits an editable AUTO-handle **Bezier** via `build_pipe(spline_type='BEZIER')`; smoothed/stroke paths drape with `segments=1` since they are already dense; pref `pipe_smooth`) + **`G` cable gravity settle** (`physics.settle_chain` of the per-span particle chain, anchors pinned, `_surface_collider` = `nearest_surface_point` push-out by the lift so the rope rests on scene geometry; Shift+Wheel becomes **slack**; prefs `cable_gravity`/`cable_slack`/`cable_collision`) + **CUSTOM profile** in the P cycle (`Scene.hardflow_profile_object`: MESH → `object_profile_points` boundary outline swept; CURVE → native `bevel_object`, non-destructive; cycle skips CUSTOM when unset, unusable sources degrade to ROUND) + mesh commits get `ensure_smooth_by_angle` polish (`HARDFLOW_OT_pipe/cable/sweep`) + **`HARDFLOW_OT_path_detail`** (Detail Along Path: instance `Scene.hardflow_detail_object` on the active curve with Array FIT_CURVE + Curve deform — chains/clips/hoses, non-destructive; F9 axis + merge) |
...
`panel` (the `Scene.hardflow_draw_mode` prop + the Curves
`Scene.hardflow_profile_object`/`hardflow_detail_object` pointers),
`translations` (the tr_TR `bpy.app.translations` catalog), plus the
`Image.hardflow_trim` / `Scene.hardflow_trim_image` pointers set inline in
`__init__`.
