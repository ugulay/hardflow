...
| `operators/push_pull.py` | SketchUp Push/Pull: raycast a face (Object) or selected faces (Edit, v1.3), drag along normal (grid snap + numeric + **vertex inference** via `snap.snap_to_candidates`), bmesh extrude w/ live snapshot/restore; `C` **Copy** (keep starting face, stacked extrude), `R` **repeat** last distance (`HARDFLOW_OT_push_pull`) |
| `operators/offset.py` | SketchUp Offset: raycast a face (Object) or selected faces (Edit, v1.3), drag to inset, bmesh inset w/ live snapshot/restore; `R` **repeat** last thickness (`HARDFLOW_OT_offset`) |
