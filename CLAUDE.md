...
| `operators/face_tool.py` | **Shared base** `_FaceDragModal` for the face-pick-drag direct-modeling tools (Push/Pull, Offset): hover-pick (maps evaluated-mesh hits past the base mesh — generative modifiers — back to a base face via `geometry.nearest_face_to_point`) + lock + drag/numeric + live snapshot/restore + snap + HUD frame + cancel/cleanup. A plain mixin (not an Operator, not registered); subclasses fill `_lock_face`/`_lock_edit`/`_update_drag`/`_refresh_preview`/`_set_value`/`_repeat_last`/`_remember_last`/`_hud_lines`/`_handle_key`. Mirrors `pipe._CurveDraw` |
| `operators/push_pull.py` | SketchUp Push/Pull (on `face_tool._FaceDragModal`): drag a face along its normal (grid snap + numeric + **vertex inference** via `snap.snap_to_candidates`), bmesh extrude w/ live snapshot/restore; `C` **Copy** (keep starting face, stacked extrude), `R` **repeat** last distance (`HARDFLOW_OT_push_pull`) |
| `operators/offset.py` | SketchUp Offset (on `face_tool._FaceDragModal`): drag to inset a face's border, bmesh inset w/ live snapshot/restore; `E` **chains into extruding** the inner face (recess / raised panel, two-phase `inset_extrude_faces`); `R` **repeat** last thickness (`HARDFLOW_OT_offset`) |
